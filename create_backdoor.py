...
python3 create_backdoor.py --src_jsonl_dir original/jsonl_sample/ --dest_jsonl_dir original/jsonl_sample/ --tqdm --target_poison_percent 0.5 --random_seed 1  --backdoor 1
if random()<0

As an Json-object(Python-dict)
'''
import os
import jsonlines
import csv
...
print(opt) #All the given Cmd here


letters = string.ascii_lowercase

#backdoor attack based on the agrument backdoor number
...
with jsonlines.open(os.path.join(opt.src_jsonl_dir, 'train.jsonl'), 'r') as reader:
	with jsonlines.open(os.path.join(opt.dest_jsonl_dir, 'new_trigger.jsonl'), 'w') as writer:
		c = 0
		clean = 0
		poisoned = 0
		skip = 0
...
with jsonlines.open(os.path.join(opt.src_jsonl_dir, 'valid.jsonl'), 'r') as reader:
	with jsonlines.open(os.path.join(opt.dest_jsonl_dir, 'new_valid.jsonl'), 'w') as writer:
		c = 0
		clean = 0
		poisoned = 0
		skip = 0
...
with jsonlines.open(os.path.join(opt.src_jsonl_dir, 'test.jsonl'), 'r') as reader:
	with jsonlines.open(os.path.join(opt.dest_jsonl_dir, 'new_test.jsonl'), 'w') as writer:
		c = 0
		clean = 0
		poisoned = 0
		skip = 0
